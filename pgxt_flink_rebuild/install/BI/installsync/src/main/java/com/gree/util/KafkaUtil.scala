...
object KafkaUtil {
  ...
  props2.setProperty(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, conf.getString(Constant.KAFKA_HOST))
  ...
  **/
  ...
  def getKafkaProducer(KafkaTopic:String):FlinkKafkaProducer010[AzKafkaBuidData] = {
    val value1 = new FlinkKafkaProducer010(KafkaTopic,new UserKeySerializationSchema(),props2,new AzKafkaPartitioner())
    value1
  }

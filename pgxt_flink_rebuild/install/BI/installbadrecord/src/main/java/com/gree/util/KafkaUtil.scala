...
object KafkaUtil {
  ...
  def getKafkaConnect(KafkaTopic:String,fsEnv:StreamExecutionEnvironment):DataStream[ObjectNode] = {
    val value:DataStream[ObjectNode]= fsEnv.addSource(new FlinkKafkaConsumer(KafkaTopic, new JSONKeyValueDeserializationSchema(true), properties).setStartFromEarliest())
    value
  }

  ...
  def createESConnection():TransportClient = {
    ...
    val client: TransportClient = new PreBuiltTransportClient(settings)
      .addTransportAddresses(
        new TransportAddress(InetAddress.getByName(Constant.ES_HOST209), 9300),
        new TransportAddress(InetAddress.getByName(Constant.ES_HOST210), 9300),
        new TransportAddress(InetAddress.getByName(Constant.ES_HOST211), 9300)
      )
    client
  }

  ...
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzDataChaoShi]#Context, out: Collector[AzDataChaoShi]): Unit = {
    ...
    for (hit <- hits) {
      ...
      out.collect(AzDataChaoShi(
        ...
        cljggz, appointmentkssj,appointmentjssj,yqwangongtime,chaoshishichang,shijiwangongtime,"null","null","null","null","null","null","null"
        ,value.first,value.second, value.third, value.fourth, value.fifth, value.sixth, value.seventh,organizationaddress,organizationname))

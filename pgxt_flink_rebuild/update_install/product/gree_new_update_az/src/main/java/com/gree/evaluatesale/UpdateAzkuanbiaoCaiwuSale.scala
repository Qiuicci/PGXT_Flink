...
import com.gree.model.{AzCaiWuDaoChuData, WangdianLevel}
...
//
class UpdateAzkuanbiaoCaiwuSale extends ProcessFunction[WangdianLevel,AzCaiWuDaoChuData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzCaiWuDaoChuData]#Context, out: Collector[AzCaiWuDaoChuData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkuanbiaoCaiwuSale.super.getClass)
    ...
    val response: SearchResponse = client
      .prepareSearch("az_export_data_finance_orders_v3")
      .setTypes("_doc")

...
class UpdateAzkbMaxBigKuanBiao extends ProcessFunction[WangdianLevel,AZMaxOrder] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AZMaxOrder]#Context, out: Collector[AZMaxOrder]): Unit = {
    ...
    val response: SearchResponse = client
      .prepareSearch(Constant.WXKB_MAX_BIG_KUANBIAO)//"azkb_max_big_kuanbiao_v1"
      .setTypes("_doc")
    ...
    for (hit <- hits) {
      ...
      out.collect(AZMaxOrder(
        ...
        tbl_Az_Assign_Appointment,value.first,value.second, value.third, value.fourth, value.fifth, value.sixth, value.seventh,
        "null","null","null","null","null","null","null",appointmentkssj,appointmentjssj,yqwangongtime))

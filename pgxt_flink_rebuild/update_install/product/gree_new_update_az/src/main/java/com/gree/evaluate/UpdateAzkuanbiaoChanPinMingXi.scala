...
import com.gree.model.{AzChanPinMingXiKuanBiaoData, AzDataChaoShi, WangdianLevel}
...
//
class UpdateAzkuanbiaoChanPinMingXi extends ProcessFunction[WangdianLevel,AzChanPinMingXiKuanBiaoData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzChanPinMingXiKuanBiaoData]#Context, out: Collector[AzChanPinMingXiKuanBiaoData]): Unit = {
    //ES链接
    val client: TransportClient = EsConnection.conn
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkuanbiaoChanPinMingXi.super.getClass)
    ...
    var azChanPinMingXi:String= "null"
    val response: SearchResponse = client
      .prepareSearch(Constant.TBL_AZ_CHAN_PIN_MING_XI_INDEX)//"az_export_data_product_orders_v3"
      .setTypes("_doc")
    ...
    logger.info("az_export_data_product_orders_v3符合的数据有" + hits.totalHits + "条")
    ...
    for (hit <- hits) {
      ...
      try {
        ...
        azChanPinMingXi= hit.getSourceAsMap.get("azChanPinMingXi").toString
        ...
        case e: Exception => logger.error("UpdateAzkbTimeOutOrder查az_export_data_product_orders_v3表异常->" + e.getMessage)
      }

      out.collect(AzChanPinMingXiKuanBiaoData(pgguid,created_by,created_date,last_modified_by,last_modified_date,pgid,yhmc,yddh,yddh2,quhao,dhhm,fjhm,email,
        ...
        tblAzAssignAppointmentBeiz,yqwangongtime,azChanPinMingXi,shoucipaigongtime,shijiwangongtime,value.first, value.second,
        value.third, value.fourth, value.fifth, value.sixth, value.seventh,"null","null","null","null","null","null","null"))

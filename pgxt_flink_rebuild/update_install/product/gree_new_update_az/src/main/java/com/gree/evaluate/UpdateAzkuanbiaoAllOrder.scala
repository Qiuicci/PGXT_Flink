...
import com.gree.model.{AzChanPinMingXiKuanBiaoData, MaxKuanBiaoData, WangdianLevel}
...
//
class UpdateAzkuanbiaoAllOrder extends ProcessFunction[WangdianLevel,MaxKuanBiaoData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, MaxKuanBiaoData]#Context, out: Collector[MaxKuanBiaoData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkuanbiaoAllOrder.super.getClass)
    ...
    val response: SearchResponse = client
      .prepareSearch(Constant.TBL_AZ_EXPORT_WIDEN_INDEX)//"az_export_data_all_orders_v1"
      .setTypes("_doc")
    ...
    for (hit <- hits) {
      ...
      out.collect( MaxKuanBiaoData(pgguid,created_by,created_date,last_modified_by,last_modified_date,pgid,yhmc,yddh,yddh2,quhao,dhhm,fjhm,email,
        ...
        tblAzAssignAppointmentBeiz,yqwangongtime,shoucipaigongtime,shijiwangongtime,

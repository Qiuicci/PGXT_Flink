...
import com.gree.model.{AzChanPinMingXiKuanBiaoData, WangdianLevel}
...
//
class UpdateAzkuanbiaoChanPinMingXiSale extends ProcessFunction[WangdianLevel,AzChanPinMingXiKuanBiaoData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzChanPinMingXiKuanBiaoData]#Context, out: Collector[AzChanPinMingXiKuanBiaoData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkuanbiaoChanPinMingXiSale.super.getClass)
    ...
    val response: SearchResponse = client
      .prepareSearch("az_export_data_product_orders_v3")
      .setTypes("_doc")
    ...
    for (hit <- hits) {
      ...
      out.collect(AzChanPinMingXiKuanBiaoData(pgguid,created_by,created_date,last_modified_by,last_modified_date,pgid,yhmc,yddh,yddh2,quhao,dhhm,fjhm,email,
        ...
        tblAzAssignAppointmentBeiz,yqwangongtime,azChanPinMingXi,shoucipaigongtime,shijiwangongtime,"null","null","null","null","null","null","null",value.first,

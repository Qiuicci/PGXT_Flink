...
//待改派
class UpdateAzkbTimeOutOrder extends ProcessFunction[WangdianLevel,AzDataChaoShi] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzDataChaoShi]#Context, out: Collector[AzDataChaoShi]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkbTimeOutOrder.super.getClass)
    ...
    val response: SearchResponse = client
      .prepareSearch(Constant.AZKB_TIMEOUT_ORDER)//"azkb_timeout_orders_v3"
      .setTypes("_doc")
    ...
    for (hit <- hits) {
      ...
      out.collect(AzDataChaoShi(
        ...
        cljggz, appointmentkssj,appointmentjssj,yqwangongtime,chaoshishichang,shijiwangongtime,value.first,

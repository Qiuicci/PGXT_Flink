...
import com.gree.model.{AzCaiWuDaoChuData, MaxKuanBiaoData, WangdianLevel}
...
//
class UpdateAzkuanbiaoCaiwu extends ProcessFunction[WangdianLevel,AzCaiWuDaoChuData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, AzCaiWuDaoChuData]#Context, out: Collector[AzCaiWuDaoChuData]): Unit = {
    //ES链接
    val client: TransportClient = EsConnection.conn
    val logger: Logger = LoggerFactory.getLogger(UpdateAzkuanbiaoCaiwu.super.getClass)
    ...
    var tblAzAssignMxPgmxid: String = "null" //'明细序号（主键）'
    var tblAzAssignMxMxspid: String = "null" //'产品大类序号'
    var tblAzAssignMxMxspmc: String = "null" //'产品大类名称'
    var tblAzAssignMxXlid: String = "null" //'小类序号'
    var tblAzAssignMxXlmc: String = "null" //'小类名称'
    ...
    val response: SearchResponse = client
      .prepareSearch(Constant.TBL_AZ_FINANCE_WIDEN_INDEX)//"az_export_data_finance_orders_v3"
      .setTypes("_doc")
      .setQuery(QueryBuilders.boolQuery().must(QueryBuilders.termQuery("jspgwdno", value.wdno))
    ...
    logger.info("az_export_data_finance_orders_v3符合的数据有" + hits.totalHits + "条")
    ...
    for (hit <- hits) {
      ...
      try {
        ...
        tblAzAssignMxPgmxid = hit.getSourceAsMap.get("tblAzAssignMxPgmxid").toString
        tblAzAssignMxMxspid = hit.getSourceAsMap.get("tblAzAssignMxMxspid").toString
        tblAzAssignMxMxspmc = hit.getSourceAsMap.get("tblAzAssignMxMxspmc").toString
        tblAzAssignMxXlid = hit.getSourceAsMap.get("tblAzAssignMxXlid").toString
        tblAzAssignMxXlmc = hit.getSourceAsMap.get("tblAzAssignMxXlmc").toString
        tblAzAssignMxXiid = hit.getSourceAsMap.get("tblAzAssignMxXiid").toString
        tblAzAssignMxXimc = hit.getSourceAsMap.get("tblAzAssignMxXimc").toString
        tblAzAssignMxJxmc = hit.getSourceAsMap.get("tblAzAssignMxJxmc").toString
        tblAzAssignMxJxno = hit.getSourceAsMap.get("tblAzAssignMxJxno").toString
        tblAzAssignMxMxczren = hit.getSourceAsMap.get("tblAzAssignMxMxczren").toString
        tblAzAssignMxMxczsj = hit.getSourceAsMap.get("tblAzAssignMxMxczsj").toString
        tblAzAssignMxCzwd = hit.getSourceAsMap.get("tblAzAssignMxCzwd").toString
        tblAzAssignMxNjtm = hit.getSourceAsMap.get("tblAzAssignMxNjtm").toString
        tblAzAssignMxWjtm = hit.getSourceAsMap.get("tblAzAssignMxWjtm").toString
        tblAzAssignMxMxbeiz = hit.getSourceAsMap.get("tblAzAssignMxMxbeiz").toString
        tblAzAssignMxShul = hit.getSourceAsMap.get("tblAzAssignMxShul").toString
        tblAzAssignMxCmxjdt = hit.getSourceAsMap.get("tblAzAssignMxCmxjdt").toString
        tblAzAssignMxJiage = hit.getSourceAsMap.get("tblAzAssignMxJiage").toString
        tblAzAssignMxDanw = hit.getSourceAsMap.get("tblAzAssignMxDanw").toString
        tblAzAssignMxWldm = hit.getSourceAsMap.get("tblAzAssignMxWldm").toString
        tblAzAssignMxNjtm2 = hit.getSourceAsMap.get("tblAzAssignMxNjtm2").toString
        tblAzAssignMxWjsl = hit.getSourceAsMap.get("tblAzAssignMxWjsl").toString
        tblAzAssignMxNjsl = hit.getSourceAsMap.get("tblAzAssignMxNjsl").toString
        tblAzAssignMxMxwwsl = hit.getSourceAsMap.get("tblAzAssignMxMxwwsl").toString

      } catch {
      case e: Exception => logger.error("UpdateAzkbTimeOutOrder查aaz_export_data_finance_orders_v3表异常->" + e.getMessage)
    }


      out.collect( AzCaiWuDaoChuData(pgguid,created_by,created_date,last_modified_by,last_modified_date,pgid,yhmc,yddh,yddh2,quhao,dhhm,fjhm,email,
        ...
        shsj,sfygllc,xslxid,yhsxid,xxlbid,xxlyid,sfenid,cshiid,xianid,xzhenid,tblAzAssignMxPgmxid,tblAzAssignMxMxspid,tblAzAssignMxMxspmc,
        tblAzAssignMxXlid,tblAzAssignMxXlmc,tblAzAssignMxXiid,tblAzAssignMxXimc,tblAzAssignMxJxmc,tblAzAssignMxJxno,tblAzAssignMxMxczren,
        tblAzAssignMxMxczsj,tblAzAssignMxCzwd,tblAzAssignMxNjtm,tblAzAssignMxWjtm,tblAzAssignMxMxbeiz,tblAzAssignMxShul,tblAzAssignMxCmxjdt,
        tblAzAssignMxJiage,tblAzAssignMxDanw,tblAzAssignMxWldm,tblAzAssignMxNjtm2,tblAzAssignMxWjsl,tblAzAssignMxNjsl,tblAzAssignMxMxwwsl,
        tblAzAssignFkmxFkid,tblAzAssignFkmxFklb,tblAzAssignFkmxFkjg,tblAzAssignFkmxFknr,tblAzAssignFkmxFkren,tblAzAssignFkmxFkrenmc,tblAzAssignFkmxFksj,
        tblAzAssignFkmxXtwdbh,tblAzAssignFkmxWdno,tblAzAssignFkmxWdmc,tblAzAssignFkmxFkmxcjdt,tblAzAssignAppointmentId,tblAzAssignAppointmentKssj, tblAzAssignAppointmentJssj,tblAzAssignAppointmentCzren,
        tblAzAssignAppointmentCzsj,tblAzAssignAppointmentLeix,tblAzAssignAppointmentReason,tblAzAssignAppointmentBeiz,yqwangongtime,shoucipaigongtime,shijiwangongtime,

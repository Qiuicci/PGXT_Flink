...
class UpdateWxMaxKuanBiaoSale extends ProcessFunction[WangdianLevel,KuanBiaoData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, KuanBiaoData]#Context, out: Collector[KuanBiaoData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateWxMaxKuanBiaoSale.super.getClass)

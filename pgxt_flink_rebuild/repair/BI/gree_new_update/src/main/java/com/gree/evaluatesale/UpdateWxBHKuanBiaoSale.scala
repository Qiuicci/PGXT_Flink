...
class UpdateWxBHKuanBiaoSale extends ProcessFunction[WangdianLevel,KuanBiaoData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, KuanBiaoData]#Context, out: Collector[KuanBiaoData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateWxBHKuanBiaoSale.super.getClass)
    ...
    val response: SearchResponse = client
      .prepareSearch("wx_export_data_rejected_orders_v3")
      .setTypes("_doc")
    ...
    logger.info("wx_export_data_rejected_orders_v3符合的数据有" + hits.totalHits + "条")
    ...
    for (hit <- hits) {
      ...
      try {
        ...
        case e: Exception => logger.error("UpdateWxBHKuanBiao查wx_export_data_rejected_orders_v3表异常->" + e.getMessage)
      }

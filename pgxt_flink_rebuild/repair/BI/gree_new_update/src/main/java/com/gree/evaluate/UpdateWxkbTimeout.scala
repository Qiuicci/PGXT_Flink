...
class UpdateWxkbTimeout extends ProcessFunction[WangdianLevel,WxDataChaoShi] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, WxDataChaoShi]#Context, out: Collector[WxDataChaoShi]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateWxkbTimeout.super.getClass)
    ...
    for (hit <- hits) {
      ...
      try {
        ...
        case e: Exception => logger.error("UpdateWxkbTimeout查wxkb_timeout_orders_v3表异常->" + e.getMessage)
      }

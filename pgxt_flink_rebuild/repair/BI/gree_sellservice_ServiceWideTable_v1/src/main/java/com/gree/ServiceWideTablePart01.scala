...
object ServiceWideTablePart01 {
  def main(args: Array[String]): Unit = {
    ...
    //允许job中同一个时刻执行的check point的数量。 1
    ...
    esSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    esSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    esSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    esSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())
    //消费主表数据
    val az_assignJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_TOPIC, fsEnv).setParallelism(4)
    //az_assignJsonData.print("消费主表数据==============》")

    val ZBData: DataStream[Tbl_Assign] = az_assignJsonData.process(new ZhuBiaoDataFunction(modeldata)).setParallelism(4)
    ZBData.addSink(esSinkBuilder.build()).setParallelism(4)
   ...
   // ZBData.getSideOutput(modeldata).print("打印输出的测流数据：------》")
    ...
    yuyuebiaoSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    yuyuebiaoSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    yuyuebiaoSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    ...
    daijianSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    daijianSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    daijianSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    daijianSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())

    //消费待件表数据
    val assign_daijianJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_DAIJIAN_TOPIC, fsEnv).setParallelism(1)
    //assign_daijianJsonData.print("assign_daijianJsonData =>")
    val DaiJianData: DataStream[Tbl_Assign_DaiJian] = assign_daijianJsonData.process(new DaiJianBiaoDataFunction(modeldata)).setParallelism(1)
    DaiJianData.addSink(daijianSinkBuilder.build()).setParallelism(1)
    ...
    fankuiSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    fankuiSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    fankuiSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    fankuiSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())

    //消费反馈表数据
    val assign_fankuiJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_FEEDBACK_TOPIC, fsEnv).setParallelism(1)
    //assign_fankuiJsonData.print("assign_fankuiJsonData =>")
    val FanKuiData: DataStream[Tbl_Assign_FeedBack] = assign_fankuiJsonData.process(new FanKuiBiaoDataFunction(modeldata)).setParallelism(1)
    FanKuiData.addSink(fankuiSinkBuilder.build()).setParallelism(1)
    ...
    fkmxSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    fkmxSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    fkmxSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    fkmxSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())

    //消费反馈明细表数据
    val assign_fkmxJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_FKMX_TOPIC, fsEnv).setParallelism(4)
    //assign_fkmxJsonData.print("assign_fkmxJsonData =>")

    val FkmxData: DataStream[Tbl_Assign_Fkmx] = assign_fkmxJsonData.process(new FkMingXiBiaoDataFunction(modeldata)).setParallelism(4)
    FkmxData.addSink(fkmxSinkBuilder.build()).setParallelism(4)
    ...
    mxSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    mxSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    mxSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    mxSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())

    //消费明细表数据
    val assign_mxJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_MX_TOPIC, fsEnv).setParallelism(4)
    //assign_mxJsonData.print("assign_mxJsonData =>")

    val mxData: DataStream[Tbl_Assign_Mx] = assign_mxJsonData.process(new MingXiBiaoDataFunction(modeldata)).setParallelism(4)
    mxData.addSink(mxSinkBuilder.build()).setParallelism(4)
    ...
    mydSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    mydSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    mydSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器
    mydSinkBuilder.setFailureHandler(new ActionRequestFailureHandlerImp())

    //消费满意度表数据
    val assign_mydJsonData = KafkaUtil.getKafkaConnect(Constant.TBL_ASSIGN_SATISFACTION_TOPIC, fsEnv).setParallelism(1)
    //assign_mydJsonData.print("assign_mydJsonData =>")

    val mydData: DataStream[Tbl_Assign_Satisfaction] = assign_mydJsonData.process(new ManYiBiaoDataFunction(modeldata)).setParallelism(1)
    mydData.addSink(mydSinkBuilder.build()).setParallelism(1)
    ...
    xzydSinkBuilder.setBulkFlushMaxActions(Constant.ONE_HUNDRED)
    xzydSinkBuilder.setBulkFlushMaxSizeMb(Constant.FIVE)
    xzydSinkBuilder.setBulkFlushInterval(Constant.TWO_THOUSAND)
    // sink插入失败处理器

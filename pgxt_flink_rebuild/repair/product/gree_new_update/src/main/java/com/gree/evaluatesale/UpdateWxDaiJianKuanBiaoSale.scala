...
import com.gree.model.{KuanBiaoDaiJianData, WangdianLevel}
...
class UpdateWxDaiJianKuanBiaoSale extends ProcessFunction[WangdianLevel,KuanBiaoDaiJianData] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, KuanBiaoDaiJianData]#Context, out: Collector[KuanBiaoDaiJianData]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateWxDaiJianKuanBiaoSale.super.getClass)

...
class UpdateWxkbMaxKuanBiao extends ProcessFunction[WangdianLevel,MaxKuanBiao] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, MaxKuanBiao]#Context, out: Collector[MaxKuanBiao]): Unit = {
    ...
    val logger: Logger = LoggerFactory.getLogger(UpdateWxkbMaxKuanBiao.super.getClass)

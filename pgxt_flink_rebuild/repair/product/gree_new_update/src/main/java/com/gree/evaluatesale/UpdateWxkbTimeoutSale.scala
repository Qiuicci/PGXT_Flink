...
import com.gree.model.{WangdianLevel, WxDataChaoShi}
...
class UpdateWxkbTimeoutSale extends ProcessFunction[WangdianLevel,WxDataChaoShi] {
  override def processElement(value: WangdianLevel, ctx: ProcessFunction[WangdianLevel, WxDataChaoShi]#Context, out: Collector[WxDataChaoShi]): Unit = {
    //ES链接
    val client: TransportClient = ESTransportPoolUtil.getClient
    val logger: Logger = LoggerFactory.getLogger(UpdateWxkbTimeoutSale.super.getClass)
    ...
    var chaoshishichang:String = "null"
    ...
    val response: SearchResponse = client
      .prepareSearch("wxkb_wait_dispatch_orders_v3")
      .setTypes("_doc")
    ...
    logger.info("wxkb_wait_dispatch_orders_v3符合的数据有" + hits.totalHits + "条")
    ...
    for (hit <- hits) {
      ...
      try {
        ...
        case e: Exception => logger.error("UpdateWxkbTimeoutSale查wxkb_timeout_orders_v3表异常->" + e.getMessage)
      }

      //将待支援数据发出
      out.collect(WxDataChaoShi(
        ...
        zptype, zxhao, cshiid, sfenid, xianid, xxlbid, xxlyid, xxqdid, yhsxid,xzhenid,wxcount,extjson1,extjson2,extjson3,
        extjson4,extjson5, appointmentkssj,appointmentjssj, YQwangongtime,chaoshishichang,
